...
stackedensemble.binomial.test <- function() {
    ...
    level_one_frame = h2o.getFrame(stack@model$level_one_frame_id$name)
    ...
    stack2 <- h2o.stackedEnsemble(x = x,
                                ...
                                base_models = list(my_gbm@model_id, my_rf@model_id))
    expect_equal(is.null(stack2@model$level_one_frame_id), TRUE) #fetching level one frame is off by default
